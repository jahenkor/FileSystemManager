...
public class ManualClientUnlockImmediatelyTests {
	...
	public void testCatAllLocksSameOrder() throws Exception
	{
		...
		for(int i = 0; i < 100; i++) {
			Capture<String> file = EasyMock.newCapture();
			expect(server.readFile(capture(file))).andReturn("foo").once();
			server.unLockFile(anyStringMatchingString(file,0), anyLong(), eq(false));
			expectLastCall().once();
		}
	...
	public void testEchoAllLocksSameOrder() throws Exception
	{
		...
		for(int i = 0; i < 100; i++) {
			Capture<String> file = EasyMock.newCapture();
			server.writeFile(capture(file), eq("content"));
			expectLastCall().once();
			server.unLockFile(anyStringMatchingString(file,0), anyLong(), eq(true));
			expectLastCall().once();
		}
